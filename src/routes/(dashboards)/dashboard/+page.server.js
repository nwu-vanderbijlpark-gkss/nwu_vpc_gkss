...
export const load = async ({ locals: { supabase } }) => {
	...
	const { data: memberData } = await supabase
		.from('member')
		.select('*,event_participant(id,events(*))')
		.eq('id', user.id);
		...
		if (memberData[0].interests) {
			...
			let publicUrl = member.image
				? await supabase.storage
						.from('files')
						.getPublicUrl(member.image?.substring(member.image.indexOf('/') + 1))
...
export const actions = {
	...
	uploadImage: async ({ locals: { supabase }, request }) => {
		...
		if (member[0].image && !member[0].image.includes('avatar.png')) {
			const image_path = member[0].image.replace('files/', ''); //remove the files/
			const { data, error } = await supabase.storage.from('files').update(image_path, newImage);
			if (error) {
				console.error(error);
			} else {
				image = data.fullPath;
