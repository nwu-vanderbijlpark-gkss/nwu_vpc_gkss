...
export const load = async ({ locals: { supabase } }) => {
	//check if its october, new leaders are shown on the portal in october if they exist
	const isOctober = () => new Date().getMonth() >= 9;
	let forYear = new Date().getFullYear();
	if (isOctober) {
		forYear++; //leaders of the upcoming year
		//check if they exist for the next year
		...
		if (!team.length) {
			forYear--; //show leaders of current year
		}
	}
	...
	for (let leader of team) {
		let publicUrl = await supabase.storage
			.from('files')
			.getPublicUrl(leader.member.image.substring(leader.member.image.indexOf('/'))); //removing the first "file/"
		leader.member = { ...leader.member, image: publicUrl.data.publicUrl };
		leaders.push(leader);
	}
	return { team: leaders, forYear };
};
...
export const actions = {
	...
	addMember: async ({ locals: { supabase }, request, fetch }) => {
		...
		const { error } = await supabase
			.from('team')
			.insert({ member_id: member_id, role: role, year: year });
