...
export async function load({ params, locals: { supabase } }) {
	...
	const {
		data: { user }
	} = await supabase.auth.getUser(); //gets the currently logged in user, we need the id
	if (params.username == 'profile' && user != null) {
		//this is the current session user's profile
	...
	if (data) {
		...
		if (data.length > 0) {
			...
			const { data: topics, error } = await supabase
				.from('topic')
				.select(
					'id,content,created_at,tags,topic, member(username,image,name,surname),comment(*),topic_views(*),topic_images(image)'
				)
				.eq('author_id', data[0].id)
			...
			for (let topic of topics) {
				let publicUrl = await supabase.storage
					.from('files')
					.getPublicUrl(topic.member.image.substring(topic.member.image.indexOf('/') + 1)); //removing the first "file/"
				...
				topic = {
					...
					member: {
						...
						fullName: topic.member.name + ' ' + topic.member.surname
					}
				};
				const finalImages = [];
			...
			let publicUrl = await supabase.storage
				.from('files')
				.getPublicUrl(data[0].image.substring(data[0].image.indexOf('/') + 1)); //removing the first "file/"
