export async function load({ locals: { supabase } }) {
	...
	for (let topic of topics) {
		...
		for (let comment of topic.comment) {
			let publicUrl = await supabase.storage
				.from('files')
				.getPublicUrl(comment.member.image.substring(topic.member.image.indexOf('/'))); //removing the first "file/"
			comment = {
				...comment,
				member: {
					image: publicUrl.data.publicUrl,
					username: comment.member.username,
					fullName: comment.member.name + ' ' + comment.member.surname
				}
			};
	...
	if (user) {
		...
		const { data: sessionUser } = await supabase
			.from('member')
			.select(
				'name, surname, qualification, username, image, year_of_study, interests, gender, date_of_birth, points'
			)
			.eq('id', user.id);
