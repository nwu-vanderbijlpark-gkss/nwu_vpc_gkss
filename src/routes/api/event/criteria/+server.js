...
export const POST = async ({ locals: { supabase }, request }) => {
    ...
    const { data: insertedData, error: insertError } = await table(supabase,tableName).insert(data).select();
...
export const GET = async({locals:{supabase}, url}) => {
    ...
    const {data: criteria, error: criteriaError} = await table(supabase, tableName).select("*").eq("event_id",event_id);
