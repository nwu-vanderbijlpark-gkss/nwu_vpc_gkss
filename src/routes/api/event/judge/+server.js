...
export const POST = async({locals:{supabase}, request, url, fetch}) => {
    ...
    const {data: judge, error} = await supabase.from("event_judge").insert({
        fullname: data.name,
        email: data.email,
...
export const GET = async({locals:{supabase}, url, cookies}) => {
    ...
    if(code){
        ...
        const {data: judge, error} = await supabase.from("event_judge").select("*").eq("event_id",ev_id).eq("email",email);
        ...
        const {data: judges, error} = await supabase.from("event_judge").select("*").eq("event_id",event_id);
