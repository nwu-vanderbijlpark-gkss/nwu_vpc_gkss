...
export const POST = async ({ locals: { supabase }, url }) => {
	...
	if (!group_id) {
		return returnData(null, {
			message: "Expected request url to have '?group', which is the group id"
		});
	}

	const { data: request, error } = await table(supabase, tableName)
		.insert({
			user_id: user.id,
			group_id: group_id
		})
...
export const PUT = async ({ locals: { supabase }, request, url, fetch }) => {
	...
	if (requestId && state) {
		const { data: request, error } = await table(supabase, tableName)
			...
			.eq('id', requestId)
			.select('*,member(id,email,name,surname),event_group(name)');

		//update the participant's group_id
		const { data: up, error: upError } = await table(supabase, 'event_participant')
			.update({
				event_group_id: data.group_id
			})
			.eq('event_id', data.event_id)
			.eq('user_id', request[0].member.id)
			.select();
...
export const GET = async ({ locals: { supabase }, url }) => {
	...
	if (group_id) {
		const { data: requests, error } = await table(supabase, tableName)
			.select('id,member(name,surname,email)')
		...
		return returnData(null, {
			message: "Expected '?group={group_id}'\n group_id: Id of the group."
		});
	}
};
