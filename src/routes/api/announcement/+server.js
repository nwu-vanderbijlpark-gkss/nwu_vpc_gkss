...
export const POST = async ({ request, locals: { supabase } }) => {
	...
	const {
		data: { user }
	} = await supabase.auth.getUser(); //we need the user id
...
export const GET = async ({ url, locals: { supabase } }) => {
	...
	if (id) {
		const { data: announcement, error } = await table
			.select('id,subject,body,created_at, member(name, surname)')
			.eq('id', id)
			.single();
		if (error) {
			return json({ success: false, data: error });
	...
	const { data: announcements, error } = await table
		.select('id,subject,created_at, member(name, surname)')
