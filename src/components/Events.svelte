							...
							<div class="flex gap-5">
								<div>
									<span class="countdown font-mono text-4xl">
										<span
											style="--value: {countdowns.find((c) => c.id === event.id)?.remaining.days ||
												0};"
										></span>
									</span>
									days
								...
								<div>
									<span class="countdown font-mono text-4xl">
										<span
											style="--value: {countdowns.find((c) => c.id === event.id)?.remaining.hours ||
												0};"
										></span>
									</span>
									hours
...
{#if modalEvent}
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
		<div class="relative w-full max-w-2xl rounded-lg bg-white p-8">
			<button class="absolute right-4 top-4 text-gray-600 hover:text-gray-900" onclick={closeModal}>
				✕
			</button>
			...
			<div class="flex items-center justify-between">
				...
				<a href={`/events/${modalEvent.id}#register`} class="btn btn-primary" data-sveltekit-reload>
					Register Now
				</a>
			</div>
		</div>
